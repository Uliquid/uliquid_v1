...
contract VenusDataProvider is Exponential{
    ...
    function getMarkets(
        ...
        address WETH,
        Range memory marketRange
    ) public returns (MarketData[] memory markets) {
        
        ICtoken[] memory ctokens = comptroller.getAllMarkets();
        markets = new MarketData[](marketRange.to - marketRange.from);

        uint j = 0;
        for (uint i = marketRange.from; i < marketRange.to; i++) {
    ...
    function getUserData(
        ...
        MarketUserData[] memory markets = new MarketUserData[](marketRange.to - marketRange.from);
